...
│  │  단일 노드 바이너리                                                   │    │
│  │                                                                     │    │
│  │  합의(Simplex) + 실행(mock) + 네트워킹(simulated) 통합              │    │
│  │  검증자 4명, 인-프로세스                                              │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
└──────────────────────────────────────────────────────────────────────────────┘
```

이더리움은 합의(CL)와 실행(EL)이 **별도 프로세스**로 Engine API(JSON-RPC)를 통해 통신합니다. Krypto는 현재 합의와 실행이 하나의 프로세스에 통합되어 있으며, Phase 2에서 revm을 붙이면 구조가 이더리움과 유사해집니다.
