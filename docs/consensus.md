...
4. [Q-쿼럼과 L-쿼럼](#4-q-쿼럼과-l-쿼럼)
5. [쿼럼 수학](#5-쿼럼-수학)
6. [안전성 증명 직관](#6-안전성-증명-직관)
7. [다른 BFT 알고리즘과의 비교](#7-다른-bft-알고리즘과의-비교)
8. [Minimmit: Simplex와 무엇이 다른가](#8-minimmit-simplex와-무엇이-다른가)
9. [Commonware API 레퍼런스](#9-commonware-api-레퍼런스)
...
BFT 합의에서 "쿼럼(quorum)"은 어떤 결정을 내리기 위해 필요한 최소 투표 수입니다. Simplex는 목적이 다른 두 종류의 쿼럼을 사용합니다.

### Q-쿼럼 (Quality/Safety Quorum)

**Q-쿼럼**은 **안전성(Safety)**을 보장하기 위한 쿼럼입니다. "Q"는 "Qualified quorum" 또는 "Quality quorum"에서 유래하며, 블록 확정처럼 **번복 불가능한 결정**에 사용됩니다.

```
Q-쿼럼이 필요한 상황:
  - Notarization 인증서 생성 (블록 공증)
  - Finalization 인증서 생성 (블록 확정)

임계값: ≥ 2f + 1  (전체 중 약 2/3 초과)

왜 2f+1인가?
  비잔틴 노드 f개가 동시에 두 개의 서로 다른 결정에 참여하더라도,
  두 Q-쿼럼의 교집합에는 반드시 정직한 노드가 포함됩니다.

  Q-쿼럼 A ∩ Q-쿼럼 B ≥ (2f+1) + (2f+1) - n = 2f+1+2f+1-3f-1 = f+1
  → 교집합 f+1명 중 비잔틴이 최대 f명이므로, 정직한 노드 1명 이상 포함
  → 정직한 노드는 하나의 결정에만 서명하므로 두 상충되는 Q-쿼럼은 불가능
```

Q-쿼럼이 달성되면 그 결정은 **전체 네트워크에서 유일**하고 **번복 불가능**합니다.

### L-쿼럼 (Liveness Quorum)

**L-쿼럼**은 **활성성(Liveness)**을 보장하기 위한 쿼럼입니다. "L"은 "Liveness quorum"에서 유래하며, **진행이 막히지 않도록** 하는 최소한의 쿼럼입니다.

```
L-쿼럼이 필요한 상황:
  - Nullification 인증서 생성 (뷰 스킵, 리더 교체)

임계값: ≥ f + 1  (비잔틴 수보다 하나 더)

왜 f+1인가?
  리더가 응답하지 않을 때 뷰를 진행하려면 충분한 nullify 투표가 필요합니다.
  만약 Nullification에도 Q-쿼럼(2f+1)을 요구하면:
  - 비잔틴 f명이 nullify 투표를 보내지 않으면
  - 정직한 노드 2f+1명만으로는 Q-쿼럼 불가 (전체 n=3f+1에서 2f+1 필요)
  - 악의적인 리더 + f명 비잔틴이 네트워크를 영구 정지시킬 수 있음

  L-쿼럼(f+1)을 요구하면:
  - 정직한 노드 2f+1명 중 f+1명만 nullify를 보내도 진행 가능
  - 비잔틴 f명이 방해해도 정직한 노드만으로 충분
```

L-쿼럼은 안전성 결정에 사용하기에는 너무 약합니다 (두 개의 충돌하는 L-쿼럼이 존재할 수 있음). 오직 "진행을 허용하는 신호"로만 사용됩니다.
...
Q-쿼럼은 "이 블록이 확정되었다"는 **강한 보장**, L-쿼럼은 "다음 단계로 넘어가도 좋다"는 **약한 허가**입니다.
