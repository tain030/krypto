...
| 실행 | `state_machine` (직접 구현) | 계좌 잔액 + 토큰 이체 블록 |

## 로드맵

```
Phase 1 ✓      — Simplex 합의, 4개 시뮬레이션 검증자, mock 애플리케이션
Phase 2 (현재) — 실제 상태 머신: Transaction/Block 타입, 계좌 잔액 검증
Phase 3        — revm 기반 EVM 실행 레이어 연동
Phase 4        — 이더리움 정산 레이어 (SNARK 증명 → EIP-8079 EXECUTE 프리컴파일)
```
...
│       └── state_machine.rs  상태 머신 — Block/Transaction/Application 액터
└── docs/
    ├── overview.md         이 파일
